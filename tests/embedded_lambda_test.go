...
// for the test. Specifically, we need to add a stop signal and use a parameter
// instead of an environment variable for the port. This opens us to the chance
// that the official SDK may skew from our tests but is the only way to tests
// certain conditions without running in Lambda or using a local dev environment
// like SAM local.
func StartHandler(c chan interface{}, port string, handler lambda.Handler) {
